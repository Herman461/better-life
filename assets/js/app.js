...
function initSliders() {
    if (!document.querySelector('.advantages__items.swiper-container-initialized')) {
        advantagesSlider = new Swiper('.advantages__items', {
            spaceBetween: 20,
            speed: 800,
            loop: true,
            pagination: {
...
function activateOrDeactivateSlider() {
    if (window.innerWidth < 992) {
        initSliders()
    } else {
        if (advantagesSlider && document.querySelector('.advantages__items.swiper-container-initialized')) {
...
activateOrDeactivateSlider();

window.addEventListener('resize', activateOrDeactivateSlider)

let blockAboutSlider = new Swiper('.block-about__body', {
    spaceBetween: 20,
    speed: 800,
    autoplay: {
        delay: 2000
    ...
    autoHeight: true,
    pagination: {
        el: '.block-about__progressbar',
        type: 'progressbar'
    },
})

let reviewsSlider = new Swiper('.about-reviews__slider', {
    spaceBetween: 20,
    speed: 800,
    loop: true,
    pagination: {
    ...
    navigation: {
        nextEl: ".slide-reviews__button-next",
        prevEl: ".slide-reviews__button-prev",
        el: ".slide-reviews__dotts",
    },
    breakpoints: {
        992: {
...
let blockAboutSliderAllSlides = document.querySelectorAll('.block-about__total')
let blockAboutSliderCurrentSlides = document.querySelectorAll('.block-about__current')

if (blockAboutSliderAllSlides.length > 0) {
    for (let index = 0; index < blockAboutSliderAllSlides.length; index++) {
        const slide = blockAboutSliderAllSlides[index]
        slide.innerHTML = addZero(blockAboutSlider.slides.length)
    }
}

blockAboutSlider.on('slideChange', function() {
    if (blockAboutSliderCurrentSlides.length > 0) {
        for (let index = 0; index < blockAboutSliderCurrentSlides.length; index++) {
            const slide = blockAboutSliderCurrentSlides[index]
            slide.innerHTML = addZero(blockAboutSlider.realIndex + 1)
        }
    }
...
const introBlock = document.querySelector('.intro-page');

if (!header.classList.contains('scroll') && header.classList.contains('main') && document.documentElement.scrollTop > window.innerHeight) {
    header.classList.add('scroll')
    if (introBlock) {
        introBlock.classList.add('scroll')
    }
}
...
window.addEventListener('scroll', function () {
    ...
    if (header.classList.contains('scroll') && header.classList.contains('main') && document.documentElement.scrollTop < window.innerHeight - header.offsetHeight) {
        header.classList.remove('scroll')
        if (introBlock) {
            introBlock.classList.remove('scroll')
        }
        return
    }
...
function initSelect(select) {
    ...
    for (let index = 0; index < select.length; ++index) {
        ...
        const selectOption = selectItem.querySelectorAll('option')
        const selectOptionLength = selectOption.length

        if (selectItem.id === 'edit-service' && selectItem.length <= 1) {
            continue
        }


        const selectedOption = selectItem.querySelector('option[selected]') ? selectItem.querySelector('option[selected]') : selectItem.querySelectorAll('option')[0]
        ...
        //
        // if (selectItem.id === 'edit-service') {
        //     selectList.addEventListener('click', function(e) {
        //         if (e.target.closest('.select__item')) {
        //             console.log('click')
        //             initSelect(document.querySelector('.calculator').querySelectorAll('select'))
        //         }
        //     })
        // }
        ...
        icon.innerHTML = `
            <svg>
                <use xlink:href="assets/images/icons/icons.svg#arrow-down"></use>
            </svg                       
        `
        if (icon) {
            head.append(icon)
        ...
        head.addEventListener('click', function (e) {
            ...
            if (activeSelectHead && !activeSelectHead.isEqualNode(e.currentTarget)) {
                activeSelectHead.classList.remove('active')
                _slideUp(activeSelectHead.nextElementSibling)
            }
            if (!selectList.classList.contains('_slide') && e.target.closest('.select__head')) {
                e.currentTarget.classList.toggle('active')
                _slideToggle(selectList)
                if (item.dataset.activeSelectItem !== undefined) {
                    item.removeAttribute('data-active-select-item')
                } else {
                    item.dataset.activeSelectItem = ''
                }
            }
        })
        selectList.addEventListener('click', function (e) {
            if (e.target.closest('.select__item')) {
                ...
                let newSelectedEl = selectItem.querySelector(`option[value="${value}"]`)
                const oldSelectedEl = selectItem.querySelector('option[selected]')
...
window.addEventListener('click', function (e) {
    if (document.querySelector('.select__list._slide')) return
    const activeSelectHead = document.querySelector('.select__head.active')
    if (activeSelectHead && !e.target.closest('.select')) {
        activeSelectHead.classList.remove('active')
        activeSelectHead.closest('.select').removeAttribute('data-active-select-item')
        _slideUp(activeSelectHead.nextElementSibling)
    }
})

const selectToggler = document.querySelector('.choice__toggler')

if (selectToggler) {
    selectToggler.addEventListener('click', function (e) {
        e.stopPropagation()
        const select = document.querySelectorAll('[data-select-item]')
        if (select.length === 0) return
        if (document.querySelector('.select__list._slide')) return

        const activeSelectItem = document.querySelector('[data-active-select-item]')

        if (!activeSelectItem) {
            select[0].dataset.activeSelectItem = ''
            select[0].querySelector('.select__head').classList.add('active')
            _slideDown(select[0].querySelector('.select__head').nextElementSibling)
            return
        }
        ...
        for (let index = 0; index < select.length; index++) {
            ...
            if (item.dataset.activeSelectItem !== undefined) {
                item.querySelector('.select__head').classList.remove('active')
                _slideUp(item.querySelector('.select__head').nextElementSibling)
                item.removeAttribute('data-active-select-item')

                let nextItem = select[index + 1]
                if (!nextItem) {
                    nextItem = select[0]
                }

                nextItem.dataset.activeSelectItem = ''
                nextItem.querySelector('.select__head').classList.add('active')
                _slideDown(nextItem.querySelector('.select__head').nextElementSibling)
                return
            }
...
let flag = false
let isSecondStepActivated = false
...
if (calculators.length > 0) {
    for (let index = 0; index < calculators.length; index++) {
        ...
        const calculatorObserver = new MutationObserver(function (mutations) {
            const moveSize = document.querySelector('.move-size')

            if (moveSize && moveSize.hasChildNodes() && !moveSize.classList.contains('added')) {
                const rooms = document.querySelector('.calculator .move-size .rooms')
                moveSize.classList.add('added')
                document.querySelector('.msize').appendChild(rooms.cloneNode())
                rooms.remove()
            }

            if (!isSecondStepActivated && document.querySelector('#edit-personal-info')) {
                isSecondStepActivated = true
                initSelect(document.querySelectorAll('#edit-personal-info select'))
                document.querySelector('#back-to-calendar-btn').classList.add('calculator__back-button')
                const icon = document.createElement('span')
                icon.innerHTML = `
                    <svg>
                        <use xlink:href="assets/images/icons/icons.svg#arrow-back"></use>
                    </svg                       
                `
                document.querySelector('#back-to-calendar-btn').prepend(icon)
                // document.querySelector('.form-item-first-name label')
                console.log(document.querySelector('#edit-personal-info').querySelectorAll('.form-item'))
                // const secondStepWrapper = document.createElement("div")
                // secondStepWrapper.classList.add('calculator__second-step-wrapper')
                // secondStepWrapper.append(document.querySelector('.four_block.left-small-block'))
                const firstColumn = document.createElement('div')
                const secondColumn = document.createElement('div')
                firstColumn.classList.add('calculator__first-step-column')
                secondColumn.classList.add('calculator__second-step-column')

                // const formItems = document.querySelectorAll('#edit-personal-info .form-item')
                // for (let index = 0; index < formItems.length; index++) {
                //     const formItem = formItems[index]
                //     formItem.innerHTML = formItem.innerHTML.trim()
                // }
                document.querySelector('#edit-personal-info .form-item-first-name input').previousElementSibling.style.display = "none"

                firstColumn.append(createLabel('First Name', 'edit-first-name'))
                secondColumn.append(createLabel('Last Name', 'form-item-last-name'))

                firstColumn.append(document.querySelector('#edit-personal-info .form-item-first-name'))
                secondColumn.append(document.querySelector('#edit-personal-info .form-item-last-name'))

                firstColumn.append(createLabel('Primary phone', 'primary_phone'))
                secondColumn.append(createLabel('Additional phone (optional)', 'edit-additional-phone'))

                firstColumn.append(document.querySelector('#edit-personal-info .form-item-primary-phone'))
                secondColumn.append(document.querySelector('#edit-personal-info .form-item-additional-phone'))

                firstColumn.append(createLabel('Email', 'edit-email'))
                secondColumn.append(createLabel('Confirm email', 'edit-confirmemail'))

                firstColumn.append(document.querySelector('#edit-personal-info .form-item-email'))
                document.querySelector('#edit-personal-info .form-item-email-login').style.display = "none"

                secondColumn.append(document.querySelector('#edit-personal-info .form-item-confirmemail'))



                firstColumn.append(createLabel('City:', 'edit-confirmemail'))
                secondColumn.append(createLabel('City:', 'edit-confirmemail'))

                firstColumn.append(document.querySelector('#edit-personal-info .form-item-moving-from-city'))
                secondColumn.append(document.querySelector('#edit-personal-info .form-item-moving-to-city'))


                const smallLeftRow = document.createElement("div")
                const smallRightRow = document.createElement("div")

                smallLeftRow.classList.add('calculator__second-step-small-row')
                smallRightRow.classList.add('calculator__second-step-small-row')

                smallLeftRow.append(createLabel('State:', 'edit-moving-from-state'))
                smallLeftRow.append(createLabel('ZIP:', 'edit-moving-from-zip'))

                smallLeftRow.append(document.querySelector('#edit-personal-info .form-item-moving-from-state'))
                smallLeftRow.append(document.querySelector('#edit-personal-info .form-item-moving-from-zip'))

                smallRightRow.append(createLabel('State:', 'edit-moving-to-state'))
                smallRightRow.append(createLabel('ZIP:', 'edit-moving-to-zip'))

                smallRightRow.append(document.querySelector('#edit-personal-info .form-item-moving-to-state'))
                smallRightRow.append(document.querySelector('#edit-personal-info .form-item-moving-to-zip'))

                firstColumn.append(smallLeftRow)
                secondColumn.append(smallRightRow)

                firstColumn.append(document.querySelector('#edit-personal-info .time_block'))
                secondColumn.append(document.querySelector('#edit-personal-info .form-item-poll-info'))

                document.querySelector('#edit-personal-info .big-left-block').appendChild(firstColumn)
                document.querySelector('#edit-personal-info .big-left-block').appendChild(secondColumn)

                document.querySelector('.form-item-moving-to').closest('.four_block').style.display = "none"
            }
            if (!flag) {
                flag = true
                ...
                function initCalculatorAccordion() {
                    const titles = [{
                        text: "Moving information",
                        place: "#calc-info-steps"
                    }, {
                        text: "Notes",
                        place: "#calc-results2"
                    }]
                    ...
                    for (let index = 0; index < titles.length; index++) {
                        const titleInfo = titles[index]
                        const calculatorBlockTitle = document.createElement('button')
                        calculatorBlockTitle.classList.add('calculator-block-title')
                        calculatorBlockTitle.setAttribute('tabindex', '-1')
                        calculatorBlockTitle.dataset.spoiler = ""
                        calculatorBlockTitle.textContent = titleInfo.text

                        const icon = document.createElement('span')
                        icon.classList.add('calculator-block-title__icon')
                        icon.innerHTML = `
                            <svg>
                                <use xlink:href="assets/images/icons/icons.svg#arrow-down"></use>
                            </svg>  
                        `
                        calculatorBlockTitle.append(icon)
                        document.querySelector(titleInfo.place).before(calculatorBlockTitle)
                    }
                }
                ...
                let isEditSelectInit
                const editSelect = document.querySelector("select#edit-service")
                const editServiceSelectObserver = new MutationObserver(function (mutations) {
                    if (!isEditSelectInit) {
                        isEditSelectInit = true
                        initSelect(document.querySelectorAll('select#edit-service'))
                    }
                });
                editServiceSelectObserver.observe(editSelect, {
                    childList: true,
                    subtree: true,
                })
                // Сюда добавлять классы, чтобы они отображались
                // document.querySelector('.er-request-form').classList.add('test')
            }
        });
        calculatorObserver.observe(calculator, {
            attributeFilter: ['ng-cloak'],
            childList: true,
            subtree: true,
        })
    }
...
let isSmallCalculatorActivated = false
const ultraSmallCalculators = document.querySelectorAll('.ultra-form-wrapper')
if (ultraSmallCalculators.length > 0) {
    for (let index = 0; index < ultraSmallCalculators.length; index++) {
        const calculator = ultraSmallCalculators[index]
        const calculatorObserver = new MutationObserver(function (mutations) {
            if (!isSmallCalculatorActivated) {
                isSmallCalculatorActivated = true
                // document.querySelector('.movesizeinput').parentElement.classList.add('ultra-form-wrapper__movesize')
                const select = document.querySelectorAll('.ultra-form-wrapper select')
                if (select.length > 0) {
                    initSelect(select)
                }
            }
